	...
	return (
		<div className='container'>
			...
			<div className='main__content'>
				{isLoaded
					? rubricsItems.map((objRubric, index) => <RubricsBlock key={`${objRubric.id}_${index}`} {...objRubric} />)
					: Array(10)
							.fill(0)
							.map((_, index) => <LoadingBlock key={index} />)}
				{isLoadedM
					? MediumRubricItems.map((objMediumRubric, index) => (
							<MediumRubrickBlock key={`${objMediumRubric.id}_${index}`} {...objMediumRubric} />
					  ))
					: Array(1)
							.fill(1)
							.map((_, index) => <MediumLoadingBlock key={index} />)}
				{isLoadedB
					? BigRubricItems.map((objBigRubric, index) => <BigRubricBlock key={`${objBigRubric.id}_${index}`} {...objBigRubric} />)
					: Array(1)
							.fill(2)
